...
namespace DSJTournaments.Data
{
    internal static class Snapshotter
    {
        ...
        public class Snapshot<T>
        {
            ...
            private static Func<T, T, List<Change>> GenerateDiffer()
            {
                ...
                foreach (var prop in RelevantProperties())
                {
                    ...
                    il.EmitCall(OpCodes.Call,
                        typeof(Snapshot<T>).GetMethod("AreEqual", BindingFlags.NonPublic | BindingFlags.Static)
                            .MakeGenericMethod(prop.PropertyType), null);
                    // [result] 
            ...
            {
                var dm = new DynamicMethod("DoClone", typeof(T), new[] {typeof(T)}, true);
